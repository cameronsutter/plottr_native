...
class CharacterCustomAttributes extends Component {
  static navigationOptions = ({ navigation }) => {
    return {
      drawerLabel: 'Character Custom Attributes',
      drawerIcon: ({ tintColor, focused }) => (
        <Ionicons name={focused ? 'ios-bowtie' : 'ios-bowtie-outline'}
  ...
  addAttribute = () => {
    if (this.state.text) {
      this.props.actions.addCharacterAttr(this.state.text.trim())
    }
  ...
  deleteAttribute = (attribute) => {
    Alert.alert(
      ...
      [
        {text: 'Yes', onPress: () => {
          this.props.actions.removeCharacterAttr(attribute)
        }},
...
CharacterCustomAttributes.propTypes = {
...
function mapStateToProps (state) {
  return {
    attributes: state.customAttributes['characters'],
  }
}
